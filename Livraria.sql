...
select count(idLivro) as 'Quantidade Livros'
from livro; 

select nome as 'Nome Clientes'
from cliente order by nome asc;

select Editora.nome as 'Nome Editora', Livro.titulo as 'Titulo Livro'
from Editora
join Livro on Editora.idEditora = Livro.idEditora
order by nome desc;

select Editora.nome as 'Nome Editora', 
avg(Livro.preco) as 'Media Preço Livro'
from Editora
join Livro on Editora.idEditora = Livro.idEditora
group by nome;

select Cliente.nome as 'Nome Cliente',
sum(ItemPedido.quantidade) as 'Quantidade Livros Comprados'
from Cliente
